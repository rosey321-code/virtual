...
/* ── 3D NFC Card (Hero Section Version) ── */
...
const stats = [
  { n: 2000, s: '+', label: 'NFC Products Sold' },
  { n: 500, s: '+', label: 'Happy Businesses' },
  { n: 15, s: '+', label: 'Countries Reached' },
...
export default function Home() {
  ...
  return (
    <main>
      ...
      <section style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', position: 'relative', overflow: 'hidden', background: 'var(--dark)' }}>
        <Particles />
        <motion.div className="container" style={{ y: heroY, opacity: heroOp, position: 'relative', zIndex: 1, paddingTop: 100, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 80, alignItems: 'center' }}>
          <div>
            <Reveal><span className="tag-fire" style={{ marginBottom: 28, display: 'inline-block' }}>🔥 The Future of Networking</span></Reveal>
            <Reveal delay={0.2}><h1 style={{ fontSize: 'clamp(56px, 7vw, 96px)', marginBottom: 24, lineHeight: 0.95 }}>TAP.<br /><span className="fire-text">CONNECT.</span><br />DOMINATE.</h1></Reveal>
            <Reveal delay={0.4}><p style={{ fontSize: 18, color: 'rgba(255,255,255,0.55)', marginBottom: 40, maxWidth: 480 }}>Premium NFC stands and smart cards for modern professionals.</p></Reveal>
      ...
      {/* ══ PRODUCTS SECTION ══ */}
      <section className="section" style={{ paddingTop: '180px', paddingBottom: '100px' }}>
        <div className="container">
          <Reveal><div style={{ textAlign: 'center', marginBottom: 120 }}><h2 style={{ fontSize: 'clamp(40px, 5vw, 70px)' }}>OUR <span className="fire-text">PRODUCTS</span></h2></div></Reveal>
                ...
                >
                  <motion.img
                    ...
                    variants={{
                      ...
                      hover: { y: -200, opacity: 1, scale: 1 } 
                    }}
                    animate={hoveredIdx === i ? "hover" : "initial"}
                    transition={{ type: 'spring', stiffness: 100, damping: 15 }}
                    style={{ position: 'absolute', top: '0', left: '50%', x: '-50%', zIndex: -1, width: '320px', height: '380px', objectFit: 'contain' }}
                    alt="nfc-product"
                  />
      ...
      <section className="section" style={{ background: 'var(--dark2)' }}>
        <div className="container">
          <Reveal><div style={{ textAlign: 'center', marginBottom: 80 }}><span className="tag-fire" style={{ marginBottom: 20, display: 'inline-block' }}>How It Works</span><h2 style={{ fontSize: 'clamp(40px,5vw,72px)' }}>FROM ORDER TO<br /><span className="fire-text">FIRST TAP IN 3 DAYS</span></h2></div></Reveal>
            ...
            {steps.map((s, i) => (
              <Reveal key={s.n} delay={i * 0.15}>
                <div style={{ textAlign: 'center' }}>
                  ...
                  <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: 14 }}>{s.desc}</p>
                </div>
              </Reveal>
            ))}
          </div>
  ...
  <div style={{ textAlign: 'center', marginBottom: 72 }}>
    ...
    <h2 style={{ fontSize: 'clamp(40px, 5vw, 72px)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 24 }}>
            {testimonials.map((t, i) => (
              <Reveal key={t.name} delay={i * 0.12}>
                <div className="card-fire" style={{ padding: 36 }}>
                  <p style={{ color: 'rgba(255,255,255,0.7)', fontStyle: 'italic' }}>"{t.text}"</p>
                  <div style={{ marginTop: 24, display: 'flex', alignItems: 'center', gap: 12 }}>
                    ...
                    <div><p style={{ fontWeight: 'bold' }}>{t.name}</p><p style={{ fontSize: 12, opacity: 0.5 }}>{t.role}</p></div>
                  </div>
                </div>
              </Reveal>
            ))}
      ...
      {/* ══ CTA SECTION WITH CHARACTER IMAGE ══ */}
      <section style={{ padding: '120px 0 0 0', background: '#000', position: 'relative', overflow: 'hidden' }}>
        <Particles />
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between' }}>
            <motion.div 
                initial={{ x: -150, rotate: -5, opacity: 0 }}
            ...
            <div className="container" style={{ textAlign: 'center', paddingBottom: '120px', flex: 1 }}>
                <Reveal>
                    <h2 style={{ fontSize: 'clamp(48px,7vw,96px)', lineHeight: 1 }}>READY TO<br /><span className="fire-text">GROW YOUR BRAND?</span></h2>
                    <Link to="/contact" className="btn-fire" style={{ marginTop: 40, display: 'inline-block', padding: '15px 40px' }}>
